...
TEST_F( CameraTest, TransformTest )
{
  ...
  node->childs() += Node::create( PointLight::create( Vector3f( 1., 2., 3. ), Color3f( 1., 1., 1. )));
  ...
  EXPECT_EQ( lights->array< 0 >().size(), 1 );

  EXPECT_FLOAT_EQ( lights->array< 0 >()[ 0 ].x(), 3. );
  EXPECT_FLOAT_EQ( lights->array< 0 >()[ 0 ].y(), 5. );
  EXPECT_FLOAT_EQ( lights->array< 0 >()[ 0 ].z(), 7. );
}
