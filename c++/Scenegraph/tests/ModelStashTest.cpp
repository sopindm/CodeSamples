...
TEST_F( ModelStashTest, AllocationTest )
{
  stash->addObject( node1.get() );
  stash->addObject( node2.get() );
...
TEST_F( ModelStashTest, TransformTest )
{
  ...
  transformedVertices += Vector3f( 2.0, 1.0, 3.0 ),
    Vector3f( 3.0, .25, -3.7 );
    
  NormalArray transformedNormals;
  transformedNormals += Vector3f( 0.1, 0.6, 0.5 ).normalized(),
    Vector3f( 0.15, 1.4, 0.2 ).normalized();

  EXPECT_TRUE( stash->vertices() == transformedVertices );
  checkArray3f( stash->normals(), transformedNormals );
  ...
  transformedVertices += Vector3f( 6.0, 0.5, 2.0 ),
    Vector3f( 4.6, 0.7, 2.2 );

  transformedNormals += Vector3f( 1.4, -7.2, 2.5 ).normalized(),
    Vector3f( 0.5, 4.4, 1.3 ).normalized();
...
TEST_F( ModelStashTest, StashTypeTest )
{
  node1->setKernel( model1 );
  node1->update( 0 );

  ModelStashPtr stash( SCENEGRAPH_STASH( ModelStash, node1 ));
  ASSERT_TRUE( stash != NULL );

  const vector< GeometryStashPtr > geometry = node1->scenegraph()->geometry();
